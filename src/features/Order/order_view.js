...
export default function OrderView() {
        const { getMenuByCategory } = MenuCRUD();
    ...
    const addFBMenu = (foods, beverages) => {
        setMenus({
            ...menus, 
            foods: [...foods], 
            beverages: [...beverages]
    ...
    const addOrder = (menuSelected, qty) => {
        const orderItem = {menuSelected,qty};
        const newOrderItems = [...order.orderItems, orderItem]
        let total = 0;
    ...
    const handleGetFoodMenu = (category) => {
        ...
        console.log(`find beverage...`);           
            setOrder({
                menuList: menus.beverages
            })
        }
    }
    const handleShowingQty = (menu) => {
        setMenuSelected({
    ...
    const handleAddOrder = () => {
        addOrder(menuSelected.menuSelected, menuSelected.qty);
        setOrder({
            isShowingQty: false
        })
    }
    ...
    const handleChangeQty = (e) => {
        ...
        if (qty > 0) {
            setMenuSelected({qty: e.target.value})
        } else {
            setMenuSelected({qty: 1})
        }
    }
